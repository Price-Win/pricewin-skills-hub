...
> ⭐ **Most popular skill: [`pricewin-deal-finder`](https://www.skills.sh/price-win/pricewin-skills-hub/pricewin-deal-finder)** — standalone, no MCP server, no API keys.
>
> ```bash
...
**Is Booking.com or Agoda cheaper?** Neither wins universally — it depends on the hotel, dates, and region. Independent tests (CNBC's review of ~200,000 hotel searches) found **Agoda had the lowest rate about 34% of the time** globally, and it tends to win in **Asia-Pacific** while **Booking.com pulls ahead more often in Europe**. That is exactly why `pricewin-deal-finder` queries **both** (plus Google Hotels and OpenTravel) in a single run, shows the per-night gap, labels the cheapest source per property, and links you straight to it — so you never have to guess.
...
**Do I need API keys or an MCP server?** No. `pricewin-deal-finder` is standalone — `node` + `npx` is all it needs. The only external dependency is Chromium (~200 MB), fetched once by the install hook.
